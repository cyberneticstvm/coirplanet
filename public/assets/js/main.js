...
// ==========================
// HERO SLIDER NUMBER FIX WITH LEADING ZEROS
// ==========================

var $slider = $('.hero-slider-14');
...
$slider.on('init reInit afterChange', function (event, slick, currentSlide) {
    ...
    var index = (currentSlide === undefined ? 1 : currentSlide + 1);

    // Current slide with 01, 02, 03
    $status1.text(padZero(index));

    // Total slides also with leading zeros
    $status2.text(padZero(slick.slideCount));
});
...
// Force slick to update first slide number immediately
setTimeout(function(){
...
// });  
    ...
    if ($('.testi-card4-tab').length) {
        $('.testi-card4-tab').thTab({
            sliderTab: true,
            tabButton: '.tab-btn'
        });
    }
